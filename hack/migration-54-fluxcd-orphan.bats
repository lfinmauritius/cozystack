#!/usr/bin/env bats
# -----------------------------------------------------------------------------
# Unit tests for platform migration 54 --> 55 (remove the tenant-cluster FluxCD
...
# (hack/testdata/migration-54-fluxcd/), mocking only the cluster boundary.
#
...
#
# jq. Migration 54, unlike the seaweedfs migrations, pipes through jq. The raw
...
#
# Run with: hack/cozytest.sh hack/migration-54-fluxcd-orphan.bats
# -----------------------------------------------------------------------------

FAKEBIN="$PWD/hack/testdata/migration-54-fluxcd"
...
tenant-a some-other-app cozystack-kubernetes-application-kubevirt-kubernetes"
  ...
  # Version stamped to 55 — asserting the number, not a bare "STAMP": a wrong
...
tenant-b kubevirt-kubernetes-fluxcd cozystack-kubernetes-application-kubevirt-kubernetes-fluxcd"
  ...
  if grep -qF -- "STAMP 55" "$FAKE_CMDLOG"; then echo "unexpected STAMP 55 after stuck finalizer" >&2; return 1; fi
  rm -rf "$WORK"
}
...
tenant-b kubevirt-kubernetes-fluxcd cozystack-kubernetes-application-kubevirt-kubernetes-fluxcd"
  ...
  if grep -qF -- "STAMP 55" "$FAKE_CMDLOG"; then echo "unexpected STAMP 55 after failed verify read" >&2; return 1; fi
  rm -rf "$WORK"
}
