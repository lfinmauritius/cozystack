...
@test "Deploy ghcr.io pull-through mirror (best-effort e2e mirror)" {
  ...
  #
  # This mirror covers the kubelet image pull only. The worker OS disk no longer
  # crosses public egress at all: the suites clone it from the golden Talos image
  # seeded in cozy-public by the kubernetes-worker-image catalog enabled below,
  # which is what the Talos image-factory cache that used to sit here was for.
  #
  # The CiliumClusterwideNetworkPolicy in the manifest is deferred to point-of-use:
  # Cilium's CRDs do not exist until Cozystack is installed further down.
